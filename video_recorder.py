...
cnt = 1
loc_path = os.getcwd()
while os.path.exists(str(loc_path) + "/" + f_name + ".avi"):
	f_name = temp_name + "({})".format(cnt)
	cnt += 1

def start_AVrecording(filename=f_name):
    global video_thread
    global audio_thread
...
def start_video_recording(filename=f_name):
    global video_thread
    video_thread = VideoRecorder()
    video_thread.start()
    return filename

def start_audio_recording(filename=f_name):
    global audio_thread
    audio_thread = AudioRecorder()
    audio_thread.start()
    return filename

def stop_AVrecording(filename=f_name):
    audio_thread.stop()
...
def file_manager(filename=f_name):
